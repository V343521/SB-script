...
-- Constants
local OFFSETS_URL = "https://imtheo.lol/Offsets/Offsets.hpp"

local NodeMonsters = {
    "Angler", "Blitz", "Pinkie", "Pandemonium", "Froger", "Chainsmoker",
    "RidgeAngler", "RidgeBlitz", "RidgePinkie", "RidgePandemonium",
    "RidgeFroger", "RidgeChainsmoker", "A60", "Harbinger",
    "Pipsqueak", "A200",
    "Bleach", "Anglemonium", "Frogermonium", "Pinkimonium", "Pandesmoker", "Blitzemonium"
}

-- State
local detectedMobs = {}
local espDoorObjects = {}
local doorDiscoveryOrder = {}
local maxVisibleDoors = 3
local currentNodeMonster = "None"
local espItemObjects = {}
local espMonsterObjects = {}
local scannedRoomIds = {}  -- [room.Address] = true — stable number key, never re-scan
local pendingRoomMap = {}  -- [room.Address] = room instance, waiting for Interactables to empty
local espNodeMonsterObjects = {}

local FetchedOffsets = nil

-- Path Lock state
local pathLock = {
    fovCircle    = nil,   -- Drawing Circle: FOV ring that follows cursor
    lockCircle   = nil,   -- Drawing Circle: ring drawn around the locked door
    lockLabel    = nil,   -- Drawing Text: "LOCKED" label on the door
    dirLine      = nil,   -- Drawing Line: screen-center → door direction
    lockedKey    = nil,   -- string key into espDoorObjects of the latched door
    isLocked     = false, -- whether a door is currently latched
    m2WasPressed = false, -- previous frame M2 state for rising-edge detection
}
...
local Settings = {
    ...
    NodeMonstersEnabled = true,
    RegularMonstersEnabled = true,
    ShowEyefestationSpawns = true,
    NotificationRepeat = 2,
    NotificationDuration = 3,
    ESPEnabled = true,
    ItemsESPEnabled = true,
    ShowKeycards = true,
    ShowPasswordNotes = true,
    ShowItems = true,
    ShowCurrency = true,
    ShowTripwires = true,
    ShowLandmines = true,
    ShowTurrets = true,
    ShowMonsterLockers = true,
    ItemESPMaxDistance = 200,
    DebugESP = false,
    PathLockEnabled = false,
    PathLockFOVRadius = 150,
    PathLockSmoothing = 8,
    PathLockStrength = 2,
}

local function notifyRepeated(message, title)
    task.spawn(function()
        for i = 1, Settings.NotificationRepeat do
            pcall(function()
                notify(message, title, Settings.NotificationDuration)
            end)
            if i < Settings.NotificationRepeat then
                task.wait(0.3)
            end
        end
    end)
...
local function fetchOffsets()
    ...
    if not ok or not content or content == "" then
        warn("Pressure ESP: Failed to fetch offsets from " .. OFFSETS_URL)
        return
    end
    ...
    for line in (content .. "\n"):gmatch("([^\n]*)\n") do
        local ns = line:match("^%s*namespace%s+(%w+)%s*{")
        if ns then
            currentNs = ns
            parsed[currentNs] = parsed[currentNs] or {}
        elseif line:match("^%s*}") then
            currentNs = nil
        elseif currentNs then
            local name, hexVal = line:match("uintptr_t%s+(%w+)%s*=%s*(0x%x+)")
            if name and hexVal then
                parsed[currentNs][name] = tonumber(hexVal)
            end
        end
    end

    FetchedOffsets = parsed
    print("Pressure ESP: Offsets loaded from " .. OFFSETS_URL)
end

function CheckForMobs()
    if not Settings.DetectionEnabled then
        detectedMobs = {}
        currentNodeMonster = "None"
        for key, data in pairs(espMonsterObjects) do
            if data.text then pcall(function() data.text:Remove() end) end
            espMonsterObjects[key] = nil
        end
        return
    end

    if Settings.NodeMonstersEnabled then
        local currentDetected = {}
        for _, obj in ipairs(workspace:GetChildren()) do
            for _, name in ipairs(NodeMonsters) do
                if obj.Name == name then
                    currentDetected[name] = true
                    if not detectedMobs[name] then
                        notifyRepeated(name .. " detected!", "Node Monster")
                        currentNodeMonster = name
                        if Settings.EntityIndicatorEnabled and not espNodeMonsterObjects[name] then
                            ...
                            espNodeMonsterObjects[name] = { nameText = nameText, distText = distText, obj = obj }
                        end
                    end
                    break
                end
        ...
        for name in pairs(detectedMobs) do
            if not currentDetected[name] then
                currentNodeMonster = "None"
                if espNodeMonsterObjects[name] then
                    local d = espNodeMonsterObjects[name]
                    if d.nameText then pcall(function() d.nameText:Remove() end) end
                    if d.distText then pcall(function() d.distText:Remove() end) end
                    espNodeMonsterObjects[name] = nil
                end
            end
        end
        detectedMobs = currentDetected
        if currentNodeMonster == "None" then
            for name in pairs(detectedMobs) do
                currentNodeMonster = name
                break
            end
    ...
    if Settings.RegularMonstersEnabled then
        local activeKeys = {}
        ...
        if monstersFolder then
            for _, obj in ipairs(monstersFolder:GetChildren()) do
                if obj.Name == "DiVineRoot" then
                    local key = obj:GetFullName()
                    activeKeys[key] = true
                    if not espMonsterObjects[key] then
                        notifyRepeated("Wall Dweller detected!", "Regular Monster")
                        local text = Drawing.new("Text")
                        text.Text = "Wall Dweller"
                        ...
                        text.Center = true
                        text.Visible = false
                        espMonsterObjects[key] = { text = text, obj = obj, monsterType = "WallDweller", lastProximityNotify = 0 }
                    end
                end
            end
        end
        ...
        if roomsFolder then
            for _, room in ipairs(roomsFolder:GetChildren()) do
                ...
                if interactables then
                    local spawn = interactables:FindFirstChild("EyefestationSpawn")
                    if spawn then
                        local key = spawn:GetFullName()
                        activeKeys[key] = true
                        if not espMonsterObjects[key] then
                            -- Register spawn indicator; do NOT notify yet — wait for CanSpawn attribute
                            local text = Drawing.new("Text")
                            text.Text = "Eyefestation"
                            ...
                            text.Center = true
                            text.Visible = false
                            espMonsterObjects[key] = { text = text, obj = spawn, monsterType = "Eyefestation", lastProximityNotify = 0, notified = false }
                        else
                            -- Check if Eyefestation has confirmed spawned (CanSpawn == "Yes")
                            local entry = espMonsterObjects[key]
                            if not entry.notified then
                                local ok, canSpawn = pcall(function() return spawn:GetAttribute("CanSpawn") end)
                                if ok and canSpawn == "Yes" then
                                    notifyRepeated("Eyefestation will spawn!", "Regular Monster")
                                    entry.notified = true
                                end
                            end
                        end
...
local function getItemType(obj)
    local ok, attr = pcall(function() return obj:GetAttribute("InteractionType") end)
    if ok and attr then return attr end
    return nil
end

local function getSettingsFlag(objType)
    if objType == "KeyCard" or objType == "InnerKeyCard" or objType == "RidgeKeyCard" then
        return Settings.ShowKeycards
    elseif objType == "PasswordPaper" then
        return Settings.ShowPasswordNotes
    elseif objType == "ItemBase" or objType == "NeoStykPickup" or objType == "Battery" then
        return Settings.ShowItems
    elseif objType == "CurrencyBase" then
        return Settings.ShowCurrency
    elseif objType == "Tripwire" then
        return Settings.ShowTripwires
    elseif objType == "Landmine" then
        return Settings.ShowLandmines
    elseif objType == "Turret" then
        return Settings.ShowTurrets
    elseif objType == "MonsterLocker" then
        return Settings.ShowMonsterLockers
    end
    return false
end

-- Registers a single object as an item ESP entry if it has a recognised InteractionType.
-- Safe to call multiple times on the same object; deduplication is via espItemObjects[key].
local function processItemObject(obj)
    local objType = getItemType(obj)
    if not (objType == "KeyCard" or objType == "InnerKeyCard" or objType == "RidgeKeyCard"
        or objType == "PasswordPaper" or objType == "ItemBase" or objType == "CurrencyBase"
        or objType == "NeoStykPickup" or objType == "Battery"
        or objType == "Landmine" or objType == "Turret"
        or objType == "MonsterLocker") then return end
    local key = obj:GetFullName()
    if espItemObjects[key] then return end
    if not getSettingsFlag(objType) then return end
    local color, label
    if objType == "KeyCard" then
        color = Color3.fromRGB(0, 255, 0); label = "Keycard"
    elseif objType == "InnerKeyCard" then
        color = Color3.fromRGB(28, 57, 187); label = "Inner Key"
    elseif objType == "RidgeKeyCard" then
        color = Color3.fromRGB(169, 169, 169); label = "Ridge Key"
    elseif objType == "PasswordPaper" then
        color = Color3.fromRGB(255, 200, 0); label = "Password"
    elseif objType == "ItemBase" then
        color = Color3.fromRGB(173, 216, 230); label = obj.Name
    elseif objType == "NeoStykPickup" then
        color = Color3.fromRGB(173, 216, 230); label = "NeoStyk"
    elseif objType == "Battery" then
        color = Color3.fromRGB(173, 216, 230); label = "Battery"
    elseif objType == "CurrencyBase" then
        color = Color3.fromRGB(0, 100, 0)
        local value = obj.Name:match("Currency(%d+)")
        label = value and ("$" .. value) or "Currency"
    elseif objType == "Landmine" then
        color = Color3.fromRGB(255, 105, 180); label = "Mine"
    elseif objType == "Turret" then
        color = Color3.fromRGB(255, 165, 0); label = "Turret"
    elseif objType == "MonsterLocker" then
        color = Color3.fromRGB(128, 0, 128); label = "Fake Locker"
    end
    if not (color and label) then return end
    local text = Drawing.new("Text")
    text.Text = label
    text.Color = color
    text.Size = 16
    ...
    text.Center = true
    text.Visible = false
    espItemObjects[key] = { text = text, obj = obj, objType = objType }
end

-- ═══════════════════════════════════════════════════
-- Room Watcher — event-driven, per-room scanning
-- Signal: Interactables folder no longer contains any known placeholder names.
-- Keys are room.Address (stable number), NOT instance refs (new wrapper per GetChildren call)
-- ═══════════════════════════════════════════════════

-- Known placeholder part names that live in the Interactables folder.
-- A room is ready to scan once none of these remain.
local PLACEHOLDER_NAMES = {
    BigDrawers       = true,
    LockersOnly      = true,
    SmallDrawers     = true,
    LockersRidgeOnly = true,
    GreyOfficeDrawers = true,
    SpeakerSpawner   = true,
    LostAbstractSpawn = true,
    ItemLocker       = true,
}

-- Returns true if the room's Interactables folder still has any known placeholder parts.
local function hasActivePlaceholders(room)
    local ia = room:FindFirstChild("Interactables")
    if not ia then return false end
    local ok, ch = pcall(function() return ia:GetChildren() end)
    if not ok then return false end
    for _, child in ipairs(ch) do
        if PLACEHOLDER_NAMES[child.Name] then return true end
    end
    return false
end

local function registerRoomDoors(room)
    ...
    for _, exit in ipairs(exits:GetChildren()) do
        ...
        if part then
            ...
            if not espDoorObjects[key] then
                ...
                text.Visible = false
                espDoorObjects[key] = {
                    text = text, part = part, exit = exit,
                    discoveryTime = os.clock()
                }
                table.insert(doorDiscoveryOrder, key)
            end
        end
    end
end

-- Process a pre-fetched descendant list for item ESP. Returns found-items list for debug.
local function processDescList(desc)
    if not Settings.ItemsESPEnabled then return {} end
    local found = {}
    for _, obj in ipairs(desc) do
        local key = obj:GetFullName()
        if not espItemObjects[key] then
            processItemObject(obj)
            if espItemObjects[key] then
                table.insert(found, { objType = espItemObjects[key].objType, path = key })
            end
        end
    end
    return found
...
-- Full descendant scan for a room (used by force-rescan and watcher).
local function scanRoomItems(room)
    if not Settings.ItemsESPEnabled then return {} end
    local ok, desc = pcall(function() return room:GetDescendants() end)
    local found = processDescList(ok and desc or {})

    -- Tripwires are detected by model name, not InteractionType.
    -- Register an indicator at the ProxyPart of each "Tripwire" model in the room.
    if Settings.ShowTripwires then
        local ok2, children = pcall(function() return room:GetChildren() end)
        if ok2 then
            for _, child in ipairs(children) do
                if child.Name == "Tripwire" then
                    local key = child:GetFullName()
                    if not espItemObjects[key] then
                        local proxy = child:FindFirstChild("ProxyPart")
                        if proxy then
                            local text = Drawing.new("Text")
                            text.Text = "Tripwire"
                            ...
                            text.Center = true
                            text.Visible = false
                            espItemObjects[key] = { text = text, obj = proxy, objType = "Tripwire" }
                            table.insert(found, { objType = "Tripwire", path = key })
                        end
                    end
                end
            end
...
local function debugPrintRoom(name, addr, state, found)
    local addrStr = string.format("0x%X", addr)
    local itemStr = (found and #found > 0) and tostring(#found) .. " found" or "None"
    print(string.format("[Room] %s | %s | %s | Items: %s", name, addrStr, state, itemStr))
    if found then
        for _, item in ipairs(found) do
            print(string.format("  %s | %s", item.objType, item.path))
        end
    end
end

-- pendingRoomMap values: { room }
-- Rooms with no Interactables folder are scanned immediately (already loaded).
-- Rooms with Interactables are polled every 0.5s via hasActivePlaceholders (cheap).
-- When all known placeholders are gone, GetDescendants runs once and the room is done.

local function roomWatcher()
    while true do
        task.wait(0.5)
        local gameplayFolder = workspace:FindFirstChild("GameplayFolder")
        local rooms = gameplayFolder and gameplayFolder:FindFirstChild("Rooms")
        if rooms then

        -- Discover new rooms
        for _, room in ipairs(rooms:GetChildren()) do
            local addr = room.Address
            if not scannedRoomIds[addr] and not pendingRoomMap[addr] then
                registerRoomDoors(room)
                if not hasActivePlaceholders(room) then
                    -- No known placeholders — scan immediately
                    local found = scanRoomItems(room)
                    scannedRoomIds[addr] = true
                    if Settings.DebugESP then
                        debugPrintRoom(room.Name, addr, "Loaded", found)
                    end
                else
                    -- Known placeholders present — watch until they're gone
                    pendingRoomMap[addr] = { room = room }
                    if Settings.DebugESP then
                        debugPrintRoom(room.Name, addr, "Pending", nil)
                    end
                end
            end
        end
        ...
        -- Check pending rooms
        local toRemove = {}
        for addr, entry in pairs(pendingRoomMap) do
            local room = entry.room
            if not room.Parent then
                table.insert(toRemove, addr)
            elseif not hasActivePlaceholders(room) then
                -- All known placeholders cleared — models and items are in place, scan now
                local found = scanRoomItems(room)
                scannedRoomIds[addr] = true
                table.insert(toRemove, addr)
                if Settings.DebugESP then
                    debugPrintRoom(room.Name, addr, "Ready", found)
                end
            end
        end
        for _, addr in ipairs(toRemove) do
            pendingRoomMap[addr] = nil
        end
...
-- Force full item rescan (used by the Rescan Items button and when toggling Item ESP on)
local function scanItemESP()
    if not Settings.ItemsESPEnabled then
        for _, data in pairs(espItemObjects) do
            if data.text then pcall(function() data.text:Remove() end) end
        end
        espItemObjects = {}
        return
    ...
    if Settings.DebugESP then
        print("[ItemESP] Force rescan started")
    end
    for _, room in ipairs(rooms:GetChildren()) do
        local found = scanRoomItems(room)
        if Settings.DebugESP then
            local ia = room:FindFirstChild("Interactables")
            local ok, ch = pcall(function() return ia and ia:GetChildren() or {} end)
            local n = ok and #ch or 0
            local placeholderState = (n > 0) and ("Placeholder: True (" .. n .. ")") or "Placeholder: False"
            debugPrintRoom(room.Name, room.Address, placeholderState, found)
        end
    end
    if Settings.DebugESP then
        local total = 0
        for _ in pairs(espItemObjects) do total = total + 1 end
        print("[ItemESP] Force rescan done — " .. total .. " total items tracked")
    end
end
...
local function updateItemESP()
    ...
    for key, data in pairs(espItemObjects) do
        if not data.obj or not data.obj.Parent then
            if data.text then pcall(function() data.text:Remove() end) end
            espItemObjects[key] = nil
        else
            local shouldShow = getSettingsFlag(data.objType)
            if not shouldShow then
                data.text.Visible = false
            else
                local pos = getItemPosition(data.obj)
                if pos then
                    local inRange = true
                    if playerPos then
                        ...
                        if math.sqrt(dx*dx + dy*dy + dz*dz) > Settings.ItemESPMaxDistance then
                            inRange = false
                        end
                    end
                    if not inRange then
                        ...
                        local ok, result = pcall(function()
                            ...
                            return { pos = screenPos, visible = onScreen }
                        end)
                        if ok and result.visible then
                            data.text.Position = Vector2.new(result.pos.X, result.pos.Y - 20)
...
local function updateMonsterESP()
    ...
    if not Settings.DetectionEnabled or not Settings.RegularMonstersEnabled then
        for _, data in pairs(espMonsterObjects) do
            if data.text then data.text.Visible = false end
        end
    else
        for key, data in pairs(espMonsterObjects) do
            if not data.obj or not data.obj.Parent then
                ...
                -- Hide Eyefestation spawn indicators if the toggle is off
                if data.monsterType == "Eyefestation" and not Settings.ShowEyefestationSpawns then
                    data.text.Visible = false
                    -- continue to next entry
                else
                    local pos = nil
                    if data.monsterType == "WallDweller" then
                        local root = data.obj:FindFirstChild("HumanoidRootPart")
                        if root then
                            local ok, p = pcall(function() return root.Position end)
                            if ok then pos = p end
                        end
                    elseif data.monsterType == "Eyefestation" then
                        local ok, p = pcall(function() return data.obj.Position end)
                        if ok then pos = p end
                    end
                    ...
                    if pos then
                        ...
                        if data.monsterType == "WallDweller" and playerPos then
                            local dx = pos.X - playerPos.X
                            local dy = pos.Y - playerPos.Y
                            local dz = pos.Z - playerPos.Z
                            if math.sqrt(dx*dx + dy*dy + dz*dz) <= 100 and (now - data.lastProximityNotify) >= 5 then
                                pcall(function()
                                    notify("Wall Dweller is close!", "Warning", Settings.NotificationDuration)
                                end)
                                data.lastProximityNotify = now
                            end
                        end
    ...
    if not Settings.EntityIndicatorEnabled then
        for _, data in pairs(espNodeMonsterObjects) do
            if data.nameText then data.nameText.Visible = false end
            if data.distText then data.distText.Visible = false end
        end
    else
        for name, data in pairs(espNodeMonsterObjects) do
            local liveObj = workspace:FindFirstChild(name)
            local pos = nil
            if liveObj then
                local ok, p = pcall(function() return liveObj.Position end)
                if ok and p then pos = p end
                if not pos then
                    local ok2, primary = pcall(function() return liveObj.PrimaryPart end)
                    if ok2 and primary then
                        local ok3, p2 = pcall(function() return primary.Position end)
                        if ok3 then pos = p2 end
                    end
                end
                if not pos then
                    local part = liveObj:FindFirstChildWhichIsA("BasePart", true)
                    if part then
                        local ok2, p2 = pcall(function() return part.Position end)
            ...
            if pos then
                ...
                if playerPos then
                    ...
                    local dy = pos.Y - playerPos.Y
                    local dz = pos.Z - playerPos.Z
                    dist = math.floor(math.sqrt(dx*dx + dy*dy + dz*dz) + 0.5)
                end
                ...
                if ok2 and result.visible then
                    data.nameText.Position = Vector2.new(result.pos.X, result.pos.Y - 28)
                    data.nameText.Visible = true
                    data.distText.Text = dist .. " studs"
                    data.distText.Position = Vector2.new(result.pos.X, result.pos.Y - 12)
...
-- Force full door rescan (used by the Rescan Doors button)
function DoorScanner:scan()
    if not Settings.ESPEnabled then
        for _, data in pairs(espDoorObjects) do
            if data.text then pcall(function() data.text:Remove() end) end
        end
        espDoorObjects = {}
        doorDiscoveryOrder = {}
    ...
    if Settings.DebugESP then
        print("[DoorESP] Force scan — " .. #doorDiscoveryOrder .. " doors total")
    end
end
...
function updateDoorESP()
    ...
    -- Remove any door entries whose room has been unloaded (exit.Parent == nil).
    -- This replaces the stale-cleanup that was previously only done in DoorScanner:scan().
    local staleKeys = {}
    for key, data in pairs(espDoorObjects) do
        if not data.exit or not data.exit.Parent then
            if data.text then pcall(function() data.text:Remove() end) end
    ...
    if #staleKeys > 0 then
        ...
        for i = #doorDiscoveryOrder, 1, -1 do
            if not espDoorObjects[doorDiscoveryOrder[i]] then
                table.remove(doorDiscoveryOrder, i)
            end
        end
    end

    local total = #doorDiscoveryOrder
    local startIdx = math.max(1, total - maxVisibleDoors + 1)
    local visibleDoors = {}
    for i = startIdx, total do
        visibleDoors[doorDiscoveryOrder[i]] = true
    end

    for key, data in pairs(espDoorObjects) do
        if visibleDoors[key] and data.part and data.exit and data.exit.Parent then
            local success, result = pcall(function()
                local screenPos, onScreen = WorldToScreen(data.part.Position)
                return {pos = screenPos, visible = onScreen}
            end)

            if success and result.visible and result.pos then
                local x, y = result.pos.X, result.pos.Y
                if x >= 0 and y >= 0 and x <= 10000 and y <= 10000 then
                    data.text.Position = Vector2.new(x, y - 30)
                    data.text.Visible = true
                else
                    data.text.Visible = false
                end
...
local function updatePathLock()
    ...
    if not pathLock.fovCircle then
        local c = Drawing.new("Circle")
        c.Radius = Settings.PathLockFOVRadius
        c.Color = Color3.fromRGB(255, 255, 255)
        c.Thickness = 1
        c.NumSides = 64
        c.Filled = false
        c.Visible = false
        pathLock.fovCircle = c
    end
    if not pathLock.lockCircle then
        local c = Drawing.new("Circle")
        c.Radius = 22
        c.Color = Color3.fromRGB(0, 220, 255)
        c.Thickness = 2
        c.NumSides = 32
        c.Filled = false
        c.Visible = false
        pathLock.lockCircle = c
    end
    if not pathLock.lockLabel then
        local t = Drawing.new("Text")
        t.Text = "LOCKED"
        t.Color = Color3.fromRGB(0, 220, 255)
        t.Size = 14
        t.Center = true
        t.Outline = true
        t.Font = Drawing.Fonts.UI
        t.Visible = false
        pathLock.lockLabel = t
    end
    if not pathLock.dirLine then
    ...
    if not Settings.PathLockEnabled then
        pathLock.fovCircle.Visible = false
        pathLock.lockCircle.Visible = false
        pathLock.lockLabel.Visible = false
        pathLock.dirLine.Visible = false
        pathLock.isLocked = false
        pathLock.lockedKey = nil
        pathLock.m2WasPressed = false
        return
    end

    pathLock.fovCircle.Radius = Settings.PathLockFOVRadius
    pathLock.fovCircle.Position = mousePos
    pathLock.fovCircle.Visible = true

    local m2Now = ismouse2pressed()
    local m2Rising = m2Now and not pathLock.m2WasPressed

    if m2Rising then
        local bestKey, bestDist3DSq = nil, math.huge
        for key, data in pairs(espDoorObjects) do
            if data.part and data.exit and data.exit.Parent then
                local ok, res = pcall(function()
                    local sp, onScreen = WorldToScreen(data.part.Position)
                    return { pos = sp, onScreen = onScreen }
                end)
                if ok and res.onScreen then
                    local ddx = res.pos.X - mousePos.X
                    local ddy = res.pos.Y - mousePos.Y
                    if math.sqrt(ddx*ddx + ddy*ddy) <= Settings.PathLockFOVRadius then
                        local dist3DSq = math.huge
                        if playerPos then
                            local dp = data.part.Position
                            local ex = dp.X - playerPos.X
                            local ey = dp.Y - playerPos.Y
                            local ez = dp.Z - playerPos.Z
                            dist3DSq = ex*ex + ey*ey + ez*ez
                        end
                        if dist3DSq < bestDist3DSq then
                            bestKey = key
        ...
        pathLock.isLocked = bestKey ~= nil
        pathLock.lockedKey = bestKey
    end
    ...
    pathLock.m2WasPressed = m2Now

    if pathLock.isLocked and pathLock.lockedKey then
        local data = espDoorObjects[pathLock.lockedKey]
        if data and data.part and data.exit and data.exit.Parent then

            local proximityReleased = false
            if playerPos then
                local dp = data.part.Position
                local ex = dp.X - playerPos.X
                local ey = dp.Y - playerPos.Y
                local ez = dp.Z - playerPos.Z
                if ex*ex + ey*ey + ez*ez <= 25 then
                    local nextKey, nextDist3DSq = nil, math.huge
                    for k, d in pairs(espDoorObjects) do
                        if d.part and d.exit and d.exit.Parent then
                            local dp2 = d.part.Position
                            local ex2 = dp2.X - playerPos.X
                            local ey2 = dp2.Y - playerPos.Y
                            local ez2 = dp2.Z - playerPos.Z
                            local dist3DSq = ex2*ex2 + ey2*ey2 + ez2*ez2
                            if dist3DSq > 25 then
                                local ok2, res2 = pcall(function()
                                    local sp2, on2 = WorldToScreen(dp2)
                                    return { pos = sp2, onScreen = on2 }
                                end)
                                if ok2 and res2.onScreen then
                                    local ddx = res2.pos.X - mousePos.X
                                    local ddy = res2.pos.Y - mousePos.Y
                                    if math.sqrt(ddx*ddx + ddy*ddy) <= Settings.PathLockFOVRadius then
                                        if dist3DSq < nextDist3DSq then
                                            nextKey = k
                                            nextDist3DSq = dist3DSq
                                        end
                                    end
                                end
                            end
                    ...
                    pathLock.lockedKey = nextKey
                    pathLock.isLocked = nextKey ~= nil
                    proximityReleased = true
                end
            end

            if not proximityReleased then
                local ok, res = pcall(function()
                    local sp, onScreen = WorldToScreen(data.part.Position)
                    return { pos = sp, onScreen = onScreen }
                end)
                if ok and res.pos then
                    local sp = res.pos
                    local isOnScreen = res.onScreen
                        and sp.X >= 0 and sp.Y >= 0
                        and sp.X <= viewport.X and sp.Y <= viewport.Y

                    mousemoverel(
                        (sp.X - mousePos.X) / Settings.PathLockSmoothing * Settings.PathLockStrength,
                        (sp.Y - mousePos.Y) / Settings.PathLockSmoothing * Settings.PathLockStrength
                    )

                    local ddx = sp.X - screenCenter.X
                    local ddy = sp.Y - screenCenter.Y
                    local dLen = math.sqrt(ddx*ddx + ddy*ddy)
                    local lineEnd = sp
                    if dLen > 0 and not isOnScreen then
                        local nx, ny = ddx/dLen, ddy/dLen
                        local limX = nx > 0 and (viewport.X - screenCenter.X) or screenCenter.X
                        local limY = ny > 0 and (viewport.Y - screenCenter.Y) or screenCenter.Y
                        local scaleX = nx ~= 0 and math.abs(limX/nx) or math.huge
                        local scaleY = ny ~= 0 and math.abs(limY/ny) or math.huge
                        local edgeDist = math.min(scaleX, scaleY)
                        lineEnd = Vector2.new(
                            screenCenter.X + nx * edgeDist,
                            screenCenter.Y + ny * edgeDist
                        )
                    end

                    pathLock.dirLine.From = screenCenter
                    pathLock.dirLine.To = lineEnd
                    pathLock.dirLine.Visible = true

                    local displayPos = isOnScreen and sp or lineEnd
                    pathLock.lockCircle.Position = displayPos
                    pathLock.lockCircle.Visible = true
                    pathLock.lockLabel.Position = Vector2.new(displayPos.X, displayPos.Y - 34)
                    pathLock.lockLabel.Visible = true
                else
                    pathLock.lockCircle.Visible = false
                    pathLock.lockLabel.Visible = false
                    pathLock.dirLine.Visible = false
...
local Menu = UILib
Menu:SetMenuTitle("Pressure ESP v1.0")
Menu:SetWatermarkEnabled(true)

local watermarkActivity = Menu:RegisterActivity(function()
    return "Active Node Monster: " .. currentNodeMonster
end)

local DetectionTab = Menu:Tab("Detection")
local DetectionMainSection = DetectionTab:Section("Entity Detection")

DetectionMainSection:Toggle("Detection Enabled", true, function(value)
    Settings.DetectionEnabled = value
    if not value then
        currentNodeMonster = "None"
        detectedMobs = {}
    end
end):AddKeybind("insert", "Toggle", true)

DetectionMainSection:Toggle("Entity Indicator", true, function(value)
    Settings.EntityIndicatorEnabled = value
    if not value then
        for _, data in pairs(espNodeMonsterObjects) do
            if data.nameText then data.nameText.Visible = false end
            if data.distText then data.distText.Visible = false end
...
DetectionMainSection:Toggle("Node Monsters", true, function(value)
    Settings.NodeMonstersEnabled = value
    if not value then
        currentNodeMonster = "None"
        detectedMobs = {}
    end
...
DetectionMainSection:Toggle("Regular Monsters", true, function(value)
    Settings.RegularMonstersEnabled = value
end)

DetectionMainSection:Toggle("Eyefestation Spawns", true, function(value)
    Settings.ShowEyefestationSpawns = value
    if not value then
        for _, data in pairs(espMonsterObjects) do
            if data.monsterType == "Eyefestation" and data.text then
                data.text.Visible = false
            end
        end
    end
...
local DetectionNotifSection = DetectionTab:Section("Notifications")

DetectionNotifSection:Slider("Notification Repeat", 2, 1, 1, 5, "x", function(value)
    Settings.NotificationRepeat = value
end)

DetectionNotifSection:Slider("Notification Duration", 3, 1, 1, 10, "s", function(value)
    Settings.NotificationDuration = value
end)

local ESPTab = Menu:Tab("ESP")
local ESPMainSection = ESPTab:Section("Door ESP")

ESPMainSection:Toggle("ESP Enabled", true, function(value)
...
end):AddKeybind("home", "Toggle", true)

ESPMainSection:Slider("Max Visible Doors", maxVisibleDoors, 1, 1, 10, "", function(value)
    maxVisibleDoors = value
end)

ESPMainSection:Button("Rescan Doors", function()
    DoorScanner:scan()
    pcall(function()
        notify("Door scan complete!", "Door ESP", 2)
    end)
end)

local ESPItemSection = ESPTab:Section("Item ESP")

ESPItemSection:Toggle("Item ESP Enabled", true, function(value)
    Settings.ItemsESPEnabled = value
    if value then
        scanItemESP()
    else
        for _, data in pairs(espItemObjects) do
...
ESPItemSection:Slider("Max Distance", 200, 10, 50, 1000, " studs", function(value)
    Settings.ItemESPMaxDistance = value
end)

ESPItemSection:Toggle("Keycards", true, function(value)
    Settings.ShowKeycards = value
end)

ESPItemSection:Toggle("Password Notes", true, function(value)
    Settings.ShowPasswordNotes = value
end)

ESPItemSection:Toggle("Items", true, function(value)
    Settings.ShowItems = value
end)

ESPItemSection:Toggle("Currency", true, function(value)
    Settings.ShowCurrency = value
end)

ESPItemSection:Toggle("Tripwires", true, function(value)
    Settings.ShowTripwires = value
end)

ESPItemSection:Toggle("Landmines", true, function(value)
    Settings.ShowLandmines = value
end)

ESPItemSection:Toggle("Turrets", true, function(value)
    Settings.ShowTurrets = value
end)

ESPItemSection:Toggle("Fake Lockers", true, function(value)
    Settings.ShowMonsterLockers = value
end)

ESPItemSection:Button("Rescan Items", function()
    scanItemESP()
    pcall(function()
        notify("Item scan complete!", "Item ESP", 2)
    end)
end)

ESPItemSection:Toggle("Debug ESP", false, function(value)
    Settings.DebugESP = value
    if value then
        pcall(function() notify("ESP debug output enabled — check console", "Debug", 3) end)
    end
end)

local PathLockTab = Menu:Tab("Path Lock")
local PathLockSection = PathLockTab:Section("Path Lock")

PathLockSection:Toggle("Path Lock Enabled", false, function(value)
    Settings.PathLockEnabled = value
    if not value then
        if pathLock.fovCircle then pathLock.fovCircle.Visible = false end
        if pathLock.lockCircle then pathLock.lockCircle.Visible = false end
        if pathLock.lockLabel  then pathLock.lockLabel.Visible  = false end
        if pathLock.dirLine    then pathLock.dirLine.Visible    = false end
        pathLock.isLocked = false
        pathLock.lockedKey = nil
    end
end)

PathLockSection:Slider("FOV Radius", 150, 10, 50, 400, "px", function(value)
    Settings.PathLockFOVRadius = value
end)

PathLockSection:Slider("Smoothing", 8, 1, 1, 20, "", function(value)
    Settings.PathLockSmoothing = value
end)

PathLockSection:Slider("Strength", 2, 1, 1, 5, "x", function(value)
    Settings.PathLockStrength = value
end)

PathLockSection:Button("Refresh Offsets", function()
    task.spawn(function()
        fetchOffsets()
        if FetchedOffsets then
            pcall(function()
                notify("Offsets refreshed successfully!", "Path Lock", 2)
            end)
        else
            pcall(function()
                notify("Failed to fetch offsets.", "Path Lock", 3)
            end)
        end
    end)
end)

local settingsTab, menuSection, themingSection = Menu:CreateSettingsTab()

print("Pressure ESP & Entity Detection loaded!")
pcall(function()
    notify("Script loaded successfully!", "Pressure ESP", 3)
end)
task.spawn(fetchOffsets)

-- Scan all rooms that already exist when the script loads
do
    local gameplayFolder = workspace:FindFirstChild("GameplayFolder")
    local rooms = gameplayFolder and gameplayFolder:FindFirstChild("Rooms")
    if rooms then
...
while true do
    ...
    if currentTime - lastESPUpdate >= 0.03 then
        updateDoorESP()
        updateItemESP()
        updateMonsterESP()
        updatePathLock()
    ...
    if currentTime - lastMobCheck >= 0.1 then
        CheckForMobs()
        lastMobCheck = currentTime
    end
